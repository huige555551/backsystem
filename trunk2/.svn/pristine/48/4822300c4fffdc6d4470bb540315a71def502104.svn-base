...
//	}
	...
	*/
	 ...
	 */
	@RequestMapping("/course")
	public @ResponseBody ResponseContainer course(HttpServletRequest request) {
		try {
			String courseId = request.getParameter("courseId");
			String token=request.getParameter("token");
			User currentUser=this.getCurrentUser(token);
			//获取到课程的章节id（list）
			 Object course = newCourseService.getCourseById(courseId,currentUser.getId());
			 //TODO 
			 return addResponse(Config.STATUS_200, Config.MSG_200, course,Config.RESP_MODE_10, "");
		} catch (XueWenServiceException e) {
			return addResponse(e.getCode(), e.getMessage(), false,Config.RESP_MODE_10, "");
...
//	
	...
	public @ResponseBody ResponseContainer getGroupCoursesPc(HttpServletRequest request,QueryModel dm,String keyWords) {
		try {
			String groupId = request.getParameter("groupId");
			Pageable pageable = PageRequestTools.pageRequesMake(dm);
			...
			Page<NewCourse> newCourses = newCourseService.getGroupCoursesPcNew(pageable,groupId,keyWords,true);//newCourseService.getGroupCoursesPc(pageable,groupId);		
			ReponseDataTools.getClientReponseData(getReponseData(), newCourses);
	 ...
	 * @return
	 */
	@RequestMapping("/categoryCourses")
	public @ResponseBody ResponseContainer categoryCourses(HttpServletRequest request,QueryModel dm){
		try {
			String categoryId = request.getParameter("categoryId");
			String childCategoryId = request.getParameter("childCategoryId");
			Pageable pageable = PageRequestTools.pageRequesMake(dm);
			Page<NewCourse> newCourses = newCourseService.findByCategory(categoryId, childCategoryId, pageable);
			ReponseDataTools.getClientReponseData(getReponseData(), newCourses);
			return addPageResponse(Config.STATUS_200, Config.MSG_200, getReponseData(),Config.RESP_MODE_10, "");
		} catch (Exception e) {
			return addResponse(Config.STATUS_505, Config.MSG_505, false,Config.RESP_MODE_10, "");
	 ...
	 * @return
	 */
	@RequestMapping("/categoryCourseNotInGroup")
	public @ResponseBody ResponseContainer categoryCourseNotInGroup(HttpServletRequest request,QueryModel dm){
		try {
			String categoryId = request.getParameter("categoryId");
			String childCategoryId = request.getParameter("childCategoryId");
			String groupId=request.getParameter("groupId");
			Pageable pageable = PageRequestTools.pageRequesMake(dm);
			Page<NewCourse> newCourses = newCourseService.findByCategoryNotInGroup(categoryId, childCategoryId,groupId, pageable);
			ReponseDataTools.getClientReponseData(getReponseData(), newCourses);
			return addPageResponse(Config.STATUS_200, Config.MSG_200, getReponseData(),Config.RESP_MODE_10, "");
		} catch (Exception e) {
			return addResponse(Config.STATUS_505, Config.MSG_505, false,Config.RESP_MODE_10, "");
	...
	public @ResponseBody ResponseContainer createLesson(HttpServletRequest request,Lesson Lesson){
		try {
			...
			String token = request.getParameter("token");
			User currentUser = this.getCurrentUser(token);
			Lesson  lesson=lessonService.createLesson(title,intro,knowledgeId,order,currentUser.getId());
	...
	public @ResponseBody ResponseContainer createChapter(HttpServletRequest request,NewChapter newChapter){
		try {
			NewChapter chapter=newChapterService.createNewChapter(newChapter);
	...
	public @ResponseBody ResponseContainer createCourse(HttpServletRequest request,NewCourse newCourse){
		try {
			NewCourse course=newCourseService.createNewCourse(newCourse,null);
			return addResponse(Config.STATUS_200, Config.MSG_200, course,Config.RESP_MODE_10, "");
		} catch (Exception e) {
			return addResponse(Config.STATUS_505, Config.MSG_505, false,Config.RESP_MODE_10, "");
		}
