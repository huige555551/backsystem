...
//	}
	 ...
	 * @return
	 */
	@RequestMapping("findMyCourse/v1.3.7")
	public @ResponseBody ResponseContainer findMyCourse137(HttpServletRequest request,QueryModel dm) {
		try {
			String token = request.getParameter("token");
			User currentUser = this.getCurrentUser(token);
			Pageable pageable = PageRequestTools.pageRequesMake(dm);
			Page<GroupDynamic> groupDynamic = myService.findMyCreatedCourse137(currentUser.getId(),Config.TYPE_COURSE_GROUP,Config.TYPE_LIVE_GROUP, pageable);
			ReponseDataTools.getClientReponseData(getReponseData(), groupDynamic);
			return addPageResponse(Config.STATUS_200, Config.MSG_200, getReponseData(),Config.RESP_MODE_10, "");
		} catch (XueWenServiceException e) {
			e.printStackTrace();
	...
	public @ResponseBody ResponseContainer findMyActivity(HttpServletRequest request, QueryModel dm) {

		try {
			String token = request.getParameter("token");
			User currentUser = this.getCurrentUser(token);
			Pageable pageable = PageRequestTools.pageRequesMake(dm);
			Page<GroupDynamic> groupDynamic = myService.findMyCreatedCourse(currentUser.getId(),Config.TYPE_ACTIVITY_GROUP, pageable);
			ReponseDataTools.getClientReponseData(getReponseData(), groupDynamic);
			return addPageResponse(Config.STATUS_200, Config.MSG_200, getReponseData(),Config.RESP_MODE_10, "");
		} catch (XueWenServiceException e) {
			e.printStackTrace();
