...
//			
	...
	public @ResponseBody ResponseContainer myPcGroup(HttpServletRequest request,@PathVariable("id") String id) {
		...
		try {
			...
			if(group!=null){
				...
				this.getReponseData().setResult(new ArrayList<XueWenGroup>());
			}
	 ...
	 */
	@SuppressWarnings("unchecked")
	@RequestMapping("/one/{id}/applyPcNotSort")
	public @ResponseBody ResponseContainer findGroupApplyPage(QueryModel dm,@PathVariable("id") String id, String userName, HttpServletRequest request) {
		try {
			Pageable pageable = PageRequestTools.pageRequesMake(dm);
			Map<String, Object> map = groupService.findPcGroupApplyNotSort(id,userName,pageable);
			...
			if(map!=null){
				...
				if(getReponseData().getTotal_rows()==0){
					getReponseData().setResult(null);
					return addResponse(Config.STATUS_200, Config.MSG_200, getReponseData(), Config.RESP_MODE_10, "");
				}
				return addPageResponse(Config.STATUS_200, Config.MSG_200, getReponseData(), Config.RESP_MODE_10, "");
			}else{
				getReponseData().setResult(null);
...
//			newGroupCourseService.creatOldGroupNewGroupCourseDynamic();
	...
	public @ResponseBody ResponseContainer getJsonByGroupId(HttpServletRequest request,String token,String groupId,QueryModel dm) throws XueWenServiceException{
		User user = token==null?null:getCurrentUser(token);
		Map<String, Object> res=groupService.getJsonByGroupId(groupId, dm,user);
