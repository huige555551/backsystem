...
public class PraiseService {
	...
	public void addPraiseNotCheck(String domain,String appkey,String sourceId,String type ,String userId)throws XueWenServiceException{
		...
		try {
			sendLikeRabbit(sourceId,Config.LIKENUM_TYPE_GROUPDYNAMIC,1);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void addPraise(Praise praise,String userId)throws XueWenServiceException{
		...
		if(!isUserPraise(userId,praise.getDomain(),praise.getSourceId(),praise.getType())){
			...
			praiseRepository.save(praise);
			try {
				sendLikeRabbit(praise.getSourceId(),Config.LIKENUM_TYPE_GROUPDYNAMIC,1);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
...
//		praiseRepository.exists(id)
	 ...
	 * @throws Exception
	 */
	public void sendLikeRabbit(String sourceId,String type,int num)throws Exception{
		rabbitmqService.sendLike(sourceId, type,num);
	}
	
	/**
	 * 根据资源列表和用户id查询出来其中用户的点赞资源集合
	 * @param sourceIds
	 * @param userId
	 * @return
	 * @throws XueWenServiceException
	 */
	public List<String>  findBySourseListAndUserIdRspSourceId(List<String> sourceIds,String userId)throws XueWenServiceException{
		List<Praise> ps=praiseMongoTemplate.findBySourseListAndUserIdRspSourceId(sourceIds, userId);
		if(ps !=null && ps.size()>0){
			List<String> ss=new ArrayList<String>();
			for(Praise p:ps){
				ss.add(p.getSourceId());
			}
			return ss;
		}else{
			return null;
	 ...
	 * @throws XueWenServiceException
	 */
	public int countBySourceId(String sourceId)throws XueWenServiceException{
		return praiseRepository.countBySourceId(sourceId);
	}
