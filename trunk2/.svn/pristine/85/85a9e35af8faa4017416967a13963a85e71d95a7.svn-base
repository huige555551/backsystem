...
public class GroupCourseController extends BaseController{
	...
	public @ResponseBody ResponseContainer create(@PathVariable("id") String id, HttpServletRequest request) {
		...
		try {
			...
			if (groupCourses.getTotalElements() == 0) {
				getReponseData().setCurr_page(groupCourses.getNumber());
				...
				getReponseData().setCurr_page(groupCourses.getNumber() + 1);
			...
			getReponseData().setTotal_rows(groupCourses.getTotalElements());
			getReponseData().setPage_rows(groupCourses.getTotalPages());
			this.getReponseData().setResult(groupCourseService.toResponseGroupsCourseList(groupCourses.getContent()));
	...
	public @ResponseBody ResponseContainer getGroupCourses(String id, HttpServletRequest request) {
		...
		try {
			...
			getReponseData().setTotal_rows(groupCourses.getTotalElements());
			getReponseData().setPage_rows(groupCourses.getTotalPages());
			this.getReponseData().setResult(groupCourseService.toResponseNewGroupsCourseList(groupCourses.getContent()));
			return addPageResponse(Config.STATUS_200, Config.MSG_200, getReponseData(),Config.RESP_MODE_10, "");
		} catch (XueWenServiceException e) {
			e.printStackTrace();
			logger.error("==========业务错误，根据群组ID查询群成员列表失败============"+e);
	...
	/**
	 * 
	 * @Title: coursesToSharesPc
	 * @Description: 推荐多个课程进入小组分享  多个courseIds用,分割
	 * @param request
	 * @return ResponseContainer
	 * @throws
	 */
	@RequestMapping("/coursesToSharesPc")
	public @ResponseBody ResponseContainer coursesToSharesPc(HttpServletRequest request) {
		try {
			String groupId = request.getParameter("groupId");
			String courseIds = request.getParameter("courseIds");
			String token = request.getParameter("token");
			User user = getCurrentUser(token);
			groupCourseService.coursesToSharesPc(groupId, courseIds, user);
			return addResponse(Config.STATUS_200, Config.MSG_200, true,Config.RESP_MODE_10, "");
		} catch (XueWenServiceException e) {
			logger.error("==========业务错误，推荐多个课程进入小组分享失败============"+e);
			return addResponse(e.getCode(), e.getMessage(), false, Config.RESP_MODE_10, "");
		} catch (Exception e) {
			logger.error("==========未知错误，推荐多个课程进入小组分享失败============"+e);
