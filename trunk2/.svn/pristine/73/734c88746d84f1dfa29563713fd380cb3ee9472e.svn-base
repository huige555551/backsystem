...
public class YXTJSONHelper {
     ...
     * @return
     */
    public static JsonConfig getJsonFilter(final String... excludeKey)
    {
    	JsonConfig configNew = new JsonConfig();
    	
    	configNew.setJsonPropertyFilter(new PropertyFilter(){
            @Override
            public boolean apply(Object source, String name, Object value) {
            	
            	int len = excludeKey.length;
            	boolean result = false;
            	if(len > 0){
            		int i = 0;
                	while(len>i){            		
                		 result=result || name.equals(excludeKey[i]);
                		 if(result){
                			 return result;
                		 }else{
                		 i++;
