...
public class GroupTemplate {
	...
	public XueWenGroup findOneXuewenGroupOnlyGroupNameAndGroupNum(String id)throws XueWenServiceException{
		Query query=new Query(Criteria.where("id").is(id));
		query.fields().include("groupName").include("groupNumber");
		return mongoTemplate.findOne(query, XueWenGroup.class);
	}
	...
	/**
	 * 根据用户Id和群组Id判断用户是否为群组管理员
	 * @param groupId
	 * @param userId
	 * @return
	 * @throws XueWenServiceException
	 */
	public boolean isGroupOwnerByUserIdAndGroupId(String groupId,String userId)throws XueWenServiceException{
		Query query=new Query(Criteria.where("id").is(groupId).and("owner").is(userId));
		return mongoTemplate.exists(query, XueWenGroup.class);
	}
	
	/**
	 * 根据群组ID，查找群组只返回群组名称，群组头像，群组成员字段
	 * @author hjn
	 * @param id
	 * @param userId
	 * @return
	 * @throws XueWenServiceException
	 */
	public XueWenGroup findOneXuewenGroupRspGroupNameAndMemberAndLogoUrl(String id)throws XueWenServiceException{
		Query query=new Query(Criteria.where("id").is(id));
		query.fields().include("id").include("groupName").include("logoUrl").include("member").include("openFireGroup").include("isOpen");
		return mongoTemplate.findOne(query, XueWenGroup.class);
	}
