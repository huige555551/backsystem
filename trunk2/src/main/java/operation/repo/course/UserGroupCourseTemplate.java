...
public class UserGroupCourseTemplate {
	 ...
	 * @throws XueWenServiceException
	 */
	public void updateFavStatus(String groupCourseId,String userId)throws XueWenServiceException{
		Query query=new Query(Criteria.where("groupCourseId").is(groupCourseId).and("userId").is(userId));
		Update update=new Update();
		update.set("faved", false);
		mongoTemplate.updateMulti(query, update, UserGroupCourse.class);
	}
