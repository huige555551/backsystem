...
//	}
	...
	/**
	 * 
	 * @Title: getKnowledgeByGidOnlyVideo
	 * @Description:获取群组的知识列表(仅包含视频)
	 * @param groupId
	 * @return ResponseContainer
	 * @throws
	 */
	@RequestMapping(value = "getKnowledgeByGidOnlyVideo")
	public @ResponseBody ResponseContainer getKnowledgeByGidOnlyVideo(String groupId,QueryModel dm,String keywords) {
		try {
			Pageable pageable = PageRequestTools.pageRequesMake(dm);
			List<GroupShareKnowledge> ks = groupshareKnowledgeService.findByGroupIdAndkngNameLike(groupId,keywords);		
			Page<Knowledge> result = knowledgeService.getKnowledgeListOnlyVideo(ks,pageable);
			ReponseDataTools.getClientReponseData(getReponseData(), result);
			if(getReponseData().getTotal_rows()==0){
				getReponseData().setResult(null);
			}
	...
	public @ResponseBody ResponseContainer knowledgeList(QueryModel dm,String keywords) {
		Pageable pageable = PageRequestTools.pageRequesMake(dm);
		Page<Knowledge> ks = knowledgeService.findKngList(pageable,keywords);		
	...
	@RequestMapping(value = "updateAllRealTime", method = RequestMethod.POST)
	public @ResponseBody ResponseContainer updateAllRealTime(HttpServletRequest request) {
		try {
			
			knowledgeService.updateAllKnowledgeRealTime();;
			
			return addResponse(Config.STATUS_200, Config.MSG_200, true,
					Config.RESP_MODE_10, "");
		} catch (Exception e) {
			e.printStackTrace();
			return addResponse(Config.STATUS_500, e.getLocalizedMessage(), false,
					Config.RESP_MODE_10, "");
		}
	}
