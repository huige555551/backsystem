...
public class OssUserController extends OssController {
	...
	public @ResponseBody ResponseContainer searchbyinfo(HttpServletRequest request, QueryModel dm) {
		...
		try {
			...
			Page<User> users = userService.searchbyInfo(keyword, robot,registType, pageable);
...
//				JSONObject x = YXTJSONHelper.addAndModifyAttrJsonObject(topic, t);
	...
	public @ResponseBody ResponseContainer getMyCourse(HttpServletRequest request,QueryModelMul dm) {
		try {
			String userid = request.getParameter("userid");
			List<NewCourse> nc=newCourseService.getMyCourses(userid);
			return addResponse(Config.STATUS_200, Config.MSG_200, nc,
					Config.RESP_MODE_10, "");
		} catch (Exception e) {
			e.printStackTrace();
...
//	}
	...
	public @ResponseBody ResponseContainer findUser(HttpServletRequest request, @PathVariable("id") String id) {
		try {
			...
			ResponseUser respUser = userService.createResUser(user);//new ResponseUser(user);
...
//	}
	...
	public ResponseContainer sendNormal(HttpServletRequest request){
		try {
	 ...
	 * @return
	 */
	@RequestMapping("sendHeadline")
	public ResponseContainer sendHeadline(HttpServletRequest request){
		try {
			String title=request.getParameter("title");
			String content=request.getParameter("content");
			ztiaoPushService.sendHeadline(title, content);
			return addResponse(Config.STATUS_200, Config.MSG_200, true, Config.RESP_MODE_10, "");
		} catch (XueWenServiceException e) {		
			return addResponse(Config.STATUS_200, e.getMessage(), null,Config.RESP_MODE_10, "");
		} catch (Exception e){
	 ...
	 * @return
	 */
	@RequestMapping("getusenicknamelist")
	public @ResponseBody ResponseContainer getusenicknameList(HttpServletRequest request,QueryModelMul dm) {
		
		Pageable pageable = PageRequestTools.pageRequesMake(dm);
		Page<NewUserNickName> newUserNickName;
		
		try {
			String keywords=request.getParameter("keywords");
			newUserNickName = userService.findUserNickName(keywords, pageable);
			ReponseDataTools.getClientReponseData(getReponseData(), newUserNickName);
			this.getReponseData().setResult(newUserNickName);
			return addPageResponse(Config.STATUS_200, Config.MSG_200, getReponseData(),
					Config.RESP_MODE_10, "");
		} catch (XueWenServiceException e) {
			e.printStackTrace();
	 ...
	 * @return
	 */
	@RequestMapping("createusenickname")
	public @ResponseBody ResponseContainer createusenickname(HttpServletRequest request) {
		
		try {
			String nickname=request.getParameter("nickname");
			NewUserNickName  n=new NewUserNickName();
			n.setName(nickname);
			n.setNumber((int)userService.countUserNickName()+1);
			userService.saveUserNickName(n);
	 ...
	 * @return
	 */
	@RequestMapping("deleteusenickname")
	public @ResponseBody ResponseContainer deleteusenickname(HttpServletRequest request) {

		try {
			String nickname = request.getParameter("nickname");
			NewUserNickName n = userService.findUserNickNameBynickname(nickname);
			int i = (int) userService.countUserNickName();
			// 判断是否为最后一条数据
			if (n.getNumber() == i) {
				userService.deleteUserNickName(n);
			} else {
				// 最后一条数据
				NewUserNickName nn = userService.findUserNickNameBynumber(i);
				// 最后一条数据复制给要删除的数据
				n.setName(nn.getName());
				// 保存用户昵称
				userService.saveUserNickName(n);
				// 删除最后一条数据
				userService.deleteUserNickName(nn);
			}
	...
	public @ResponseBody ResponseContainer updateusernickname(HttpServletRequest request) {

		try {
			String id = request.getParameter("id");
			String nikename = request.getParameter("nickname");
			NewUserNickName n = userService.findUserNickNameByid(id);
	...
	public @ResponseBody ResponseContainer getMyCreated(HttpServletRequest request,String userId,QueryModel dm) throws XueWenServiceException{
		Map<String, Object> res=userService.getMyCreated(userId, dm);
		return addResponse(Config.STATUS_200, Config.MSG_200,res,Config.RESP_MODE_10, "");
	}
	/**
