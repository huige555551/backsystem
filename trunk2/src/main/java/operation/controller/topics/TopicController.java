...
public class TopicController extends BaseController {
	...
	public ResponseContainer mobileDetail(String topicId, String token) {
		try {
			User user = this.getCurrentUser(token);
			JSONObject res = topicService.mobileTopicDetail(user.getId(), topicId);
			return addResponse(Config.STATUS_200, Config.MSG_200, res, Config.RESP_MODE_10, "");
		} catch (XueWenServiceException e) {
			e.printStackTrace();
			return addResponse(e.getCode(), e.getMessage(), false, Config.RESP_MODE_10, "");
...
//	}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/updatePostCount")
	public @ResponseBody ResponseContainer updatePostCount(HttpServletRequest request) {
		try {
			String sourceId = request.getParameter("sourceId");
			String num = request.getParameter("num");
			topicService.increasingPostCount(sourceId,Integer.valueOf(num));
			return addResponse(Config.STATUS_200, Config.MSG_200,true, Config.RESP_MODE_10, "");
		} catch (XueWenServiceException e) {
			return addResponse(e.getCode(), e.getMessage(), false, Config.RESP_MODE_10, "");
		} catch (Exception e) {
