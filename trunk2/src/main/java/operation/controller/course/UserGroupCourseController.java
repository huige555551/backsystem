...
public class UserGroupCourseController extends BaseController{
	 ...
	 * @return
	 */
	@RequestMapping("/courseFav")
	public @ResponseBody ResponseContainer favCourse(HttpServletRequest request,QueryModel dm) {
		try {
			Pageable pageable = PageRequestTools.pageRequesMake(dm);
			String token = request.getParameter("token");
			User currentUser = this.getCurrentUser(token);
			Page<UserGroupCourse> favStudeds = userGroupCourseService.findUserFav(currentUser.getId(), pageable);
			ReponseDataTools.getClientReponseData(getReponseData(), favStudeds);
			this.getReponseData().setResult(userGroupCourseService.formateUserGroupCourseRspUserFavOrStudyList(favStudeds.getContent()));
			return addPageResponse(Config.STATUS_200, Config.MSG_200, getReponseData(),Config.RESP_MODE_10, "");
		} catch (XueWenServiceException e) {
	 ...
	 * @return
	 */
	@RequestMapping("/courseNewFav")
	public @ResponseBody ResponseContainer courseNewFav(HttpServletRequest request,QueryModel dm) {
		try {
			Pageable pageable = PageRequestTools.pageRequesMake(dm);
			String token = request.getParameter("token");
			User currentUser = this.getCurrentUser(token);
			Page<Fav> favs = userGroupCourseService.getMyCollect(currentUser.getId(), pageable);
			Map<String, Object> res = (userGroupCourseService.toNewCourseForFav(favs.getContent(),currentUser.getId()));
	...
	public @ResponseBody ResponseContainer like(HttpServletRequest request) {
		...
		try {
			...
			String appkey = request.getParameter("appKey");
			User currentUser = this.getCurrentUser(token);
			userGroupCourseService.addParise(currentUser, groupCourseId, groupId, appkey);
			return addResponse(Config.STATUS_200, Config.MSG_200, true,Config.RESP_MODE_10, "");
		} catch (XueWenServiceException e) {
			e.printStackTrace();
			logger.error("==========业务错误，群组课程收藏失败============"+e);
