...
public class ContactUserController extends BaseController{
	 ...
	 */
	@RequestMapping("info")
	public @ResponseBody ResponseContainer info(HttpServletRequest request,QueryModel dm,String userId){
		try {
			String token = request.getParameter("token");
			User currentUser = this.getCurrentUser(token);
			JSONObject res= contactUserService.info(currentUser,userId);
			return addResponse(Config.STATUS_200, Config.MSG_200,res,Config.RESP_MODE_10, "");
		}catch (XueWenServiceException e) {
			logger.error("获取好友信息&&是否被自己关注业务错误："+e);
			return addResponse(e.getCode(),e.getMessage(),false,Config.RESP_MODE_10,"");
		} catch (Exception e){
			logger.error("获取好友信息&&是否被自己关注系统错误："+e);
			return addResponse(Config.STATUS_505,Config.MSG_505,false,Config.RESP_MODE_10,"");
		}
	}
