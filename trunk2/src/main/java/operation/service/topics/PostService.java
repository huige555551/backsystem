...
//		}
		...
		try {
			if(!user.getId().equals(topic.getAuthorId())){
				ztiaoPushService.sendTopicPostOrSubPost(Config.PUSH_TOPICPOST_TYPE,user.getId(), user.getNickName(), user.getLogoURL(), 
						topic.getAuthorId(),topic.getSourceId(),topic.getSourceName(),"",post.getTopicId(), topic.getTitle());
			}
	...
	public SubPost replyPost(User user, SubPost subPost) throws XueWenServiceException {
		...
		try {
			...
			//给被回复发送消息
			if(!user.getId().equals(subPost.getToId())){
				ztiaoPushService.sendTopicPostOrSubPost(Config.PUSH_TOPICSUBPOST_TYPE,user.getId(), user.getNickName(), user.getLogoURL(), 
						subPost.getToId(),topic.getSourceId(),topic.getSourceName(),"",subPost.getTopicId(), topic.getTitle());
			}
			...
			if(!user.getId().equals(topic.getAuthorId())){
				ztiaoPushService.sendTopicPostOrSubPost(Config.PUSH_TOPICSUBPOSTTOAUTHER_TYPE,user.getId(), user.getNickName(), user.getLogoURL(), 
						topic.getAuthorId(),topic.getSourceId(),topic.getSourceName(),"",subPost.getTopicId(), topic.getTitle());
			}
	...
	public Post replyDrycargo(User user, Post post, String groupId) throws XueWenServiceException {
		...
		Drycargo drycargo = drycargoService.findOneById(post.getTopicId()) ;
		if (drycargo == null) {
			throw new XueWenServiceException(Config.STATUS_201, Config.MSG_201, null);
		}
		post.setAuthorId(user.getId());
		...
		post.setPosition(position);
		// 计算主楼回复次数
		drycargoService.saveReplyCount(post.getTopicId(), 1);
		
		//过滤信息
				try {
					if(!user.getId().equals(drycargo.getAuthorId())){
						ztiaoPushService.sendTopicPostOrSubPost(Config.PUSH_TOPICPOST_TYPE,user.getId(), user.getNickName(), user.getLogoURL(), 
								drycargo.getAuthorId(),drycargo.getGroup().toString(),drycargo.getGroupName(),"",post.getTopicId(), drycargo.getMessage());
					}
					rabbitmqService.sendRegexMessage(post.getPostId(), Config.TAG_TYPE_POST);
				} catch (Exception e) {
					logger.error("====发送主楼过滤信息发生错误====");
...
//		}
	...
	/**
	 * 根据postID删除主楼回复以及其下的所有副楼回复 干货
	 * 
	 * @throws XueWenServiceException
	 */
	public void deletePostByPostIdByDry(String dryId, String postId) throws XueWenServiceException {
//		Topic t=topicService.findOneById(topicId);
		if(drycargoService.findOneById(dryId)==null){
			throw new XueWenServiceException(Config.STATUS_201, "干货不存在！", null);
		}
		// 删除此主楼下所有子楼记录
		subPostTemplate.deleteSubPostByPostId(postId);
		
		// 干货的回复数统计减一
		topicService.increasingPostCountByDry(dryId, -1);
		// 删除主楼记录
		postTemplate.deletePostById(postId);
	}
	...
	public void deleteSubPostBySubPostId(String subPostId, String postId) throws XueWenServiceException {
		...
		if(subPosts !=null){
			for(int i=0;i<subPosts.size();i++){
				if(subPostId.equals(subPosts.get(i).getPost_id())){
					subPosts.remove(i);
				}	
			}
...
//			}
	 ...
	 * @throws
	 */
	public List<Post> findPosts(String topicId, int i) {
		QueryModel dm=new QueryModel();
		dm.setSort("likesCount");
		dm.setMode("DESC");
		dm.setS(i);
		Pageable pageable=PageRequestTools.pageRequesMake(dm);
		Page<Post> posts=postRepository.findByTopicIdAndType(topicId, 0,pageable);
	 ...
	 * @throws XueWenServiceException
	 */
	public Post replySubject(User user, Post post,String image) throws XueWenServiceException {
		if (post == null) {
			throw new XueWenServiceException(Config.STATUS_201, Config.MSG_201, null);
		}
		post.setAuthorId(user.getId());
		...
		try {
			//发送回复信息
			sendTopicPostOrSubPost(user,post);
		} catch (Exception e1) {
			logger.error("====发送回复通知发生错误====");
		}
		try {
			sendSubjectPostNumRabbit(post ,1);
		} catch (Exception e1) {
...
//		return post;
	 ...
	 * @throws Exception
	 */
	public void sendTopicPostOrSubPost(User user,Post post)throws Exception{
		if(Config.TYPE_TOPIC_GROUP.equals(post.getTranFlag())){
			Topic t=topicService.findOneById(post.getTopicId());
			if(t !=null && !user.getId().equals(t.getAuthorId())){
				ztiaoPushService.sendTopicPostOrSubPost(Config.PUSH_TOPICPOST_TYPE,user.getId(), user.getNickName(), user.getLogoURL(), 
						t.getAuthorId(),t.getSourceId(),t.getSourceName(),"",post.getTopicId(), t.getTitle());
			}
		}else if(Config.TYPE_XUANYE_GROUP.equals(post.getTranFlag()) || Config.TYPE_DRYCARGO_GROUP.equals(post.getTranFlag())){
			Drycargo drycargo = drycargoService.findOneById(post.getTopicId()) ;
			if(drycargo != null && !user.getId().equals(drycargo.getAuthorId())){
				ztiaoPushService.sendTopicPostOrSubPost(Config.PUSH_TOPICPOST_TYPE,user.getId(), user.getNickName(), user.getLogoURL(), 
						drycargo.getAuthorId(),drycargo.getGroup().toString(),drycargo.getGroupName(),"",post.getTopicId(), drycargo.getMessage());
			}
		}
	 ...
	 * @throws Exception
	 */
	public void sendSubjectPostNumRabbit(Post post ,int num)throws Exception{
		//发送异步回复数添加队列通知
		String type="";
		if(Config.TYPE_TOPIC_GROUP.equals(post.getTranFlag())){
			type=Config.POSTNUM_TYPE_GROUPTOPIC;
		}else if(Config.TYPE_XUANYE_GROUP.equals(post.getTranFlag())){
			type=Config.POSTNUM_TYPE_GROUPXUANYE;
		}else if(Config.TYPE_DRYCARGO_GROUP.equals(post.getTranFlag())){
			type=Config.POSTNUM_TYPE_DRYCARGO_GROUP;
		}
		rabbitmqService.sendLike(post.getTopicId(), type, num);
	}
