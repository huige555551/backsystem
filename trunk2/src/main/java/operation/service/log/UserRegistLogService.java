...
public class UserRegistLogService {
	...
	@Autowired
	private UserLoginLogService ullsLoginLogService;
	...
	public Page<UserRegistLog> search( long ctime,long etime, String vkey, String ukey, Pageable pageable) {
		if(StringUtil.isBlank(ukey)&&StringUtil.isBlank(vkey)){
			return userRegLogRepo.findByCtimeBetweenAndRefType(ctime, etime, Config.LOG_REG_TYPE_YQ,pageable);
		}		
		if(!StringUtil.isBlank(vkey)){
			return  userRegLogRepo.findByVkeyAndTime(vkey,ctime,etime,pageable);
		}
		if (!StringUtil.isBlank(ukey)) {
			return userRegLogRepo.findByUkeyAndTime(ukey,ctime,etime,Config.LOG_REG_TYPE_YQ,pageable);
		}
	    if((!StringUtil.isBlank(vkey))&&(!StringUtil.isBlank(ukey))){    	
			return userRegLogRepo.findByUkeyAndVkey(ukey,vkey,ctime,etime,Config.LOG_REG_TYPE_YQ,pageable);
		}
		return null;
	...
	public Page<UserRegistLog> searchQd(long ctime,long etime,String userKey,String adsId,Pageable pageable){
		if(StringUtil.isBlank(userKey)&&StringUtil.isBlank(adsId)){
			return userRegLogRepo.findByCtimeBetweenAndRefType(ctime,etime,Config.LOG_REG_TYPE_QD,pageable);
		}
		if(StringUtil.isBlank(userKey)&&(!StringUtil.isBlank(adsId))){
			return userRegLogRepo.findByAdSellerIdLikeAndCtimeBetweenAndRefType(adsId,ctime,etime,Config.LOG_REG_TYPE_QD,pageable);

		}
		if(StringUtil.isBlank(adsId)&&(!StringUtil.isBlank(userKey))){
			return userRegLogRepo.findByUkeyAndTime(userKey,ctime,etime,Config.LOG_REG_TYPE_QD,pageable);
		}
		if((!StringUtil.isBlank(adsId))&&(!StringUtil.isBlank(userKey))){
	...
	public void setloginTime(List<UserRegistLog> logs){
		for (UserRegistLog userRegistLog : logs) {
			if(userRegistLog.getUserId()!=null){
				UserLoginLog log=ullsLoginLogService.findOneByUserId(userRegistLog.getUserId());
				if(log!=null){
					userRegistLog.setLogintime(log.getLoginTime());
				}
	 ...
	 * @throws
	 */
	public void addByUserName(String inviteUserID, User user) {
		if(!StringUtil.isBlank(inviteUserID)){
		try {
			User vUser=userService.findOne(inviteUserID);
			if(vUser!=null&&user!=null){
				UserRegistLog userRegistLog=new UserRegistLog(vUser,user);
				save(userRegistLog);
			}
		} catch (Exception e) {
			e.printStackTrace();
