...
public class NewChapterService {
	 ...
	 * @throws
	 */
	public NewChapter save(NewChapter chapter) {
	...
	/**
	 * 根据chapter的ID集合删除章节集合,以及章节下所有的课时，不包括课时下所有的知识
	 * @param chapterIds
	 * @throws XueWenServiceException
	 */
	public void deleteByChapterIdsNoKng(List<Object> chapterIds)throws XueWenServiceException{
		//根据章节ID集合列表获取课时ID集合
		List<NewChapter> newChapters=findChapterListByIdListRspOnlyIdAndLessonIds(chapterIds);
		//获取所有的章节下所有的课时id集合
		List<Object> allLessonIds=getLessonIdsByNewChapters(newChapters);
...
//	}
	...
	public NewChapter createNewChapter(NewChapter newChapter,String lessonIds)throws XueWenServiceException{
		...
		for (int i = 0; i < ids.length; i++) {
			Lesson lesson=lessonService.findOneLessonBasicInfoById(ids[i]);
			lesson.setIsUsed(2);
	...
	/**
	 * 
	 * @Title: findUnusedChapter
	 * @Description: 章节垃圾数据
	 * @param pageable
	 * @return Page<NewChapter>
	 * @throws
	 */
	public Page<NewChapter> findUnusedChapter(Pageable pageable){
		return chapterRepository.findByIsUsed(1, pageable);
	}
	/**
	 * 
	 * @Title: findOne
	 ...
	 * @throws
	 */
	public NewChapter findOne(String chapterId){
		return chapterRepository.findOne(chapterId);
	}
	
	public void update(NewChapter newChapter) throws XueWenServiceException{
		if (tools.StringUtil.isBlank(newChapter.getId())) {
			throw new  XueWenServiceException(Config.STATUS_201,"章节ID不可以空",null);
		}
		NewChapter chapter=findOne(newChapter.getId());
		chapter.setTitle(newChapter.getTitle());
		chapter.setOrder(newChapter.getOrder());
		save(chapter);
	}
