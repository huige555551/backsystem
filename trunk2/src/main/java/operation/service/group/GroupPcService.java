...
public class GroupPcService {
	...
	private String groupCateSupo;
	
	@Value("${group.cate.sec}")
	private String groupCateSec;
	@Autowired
	...
	public JSONObject findGroup(String id,String userId)
		...
		if (null == one) {
			...
			String[] includeKey = {"id","groupName","intro","logoUrl","memberCount","tagNames","groupNumber","isOpen"};
...
//	}
	...
	public XueWenGroup createGroupPc(XueWenGroup group, User user,String tagName)throws XueWenServiceException {
		...
		if (!groupMongoTemplate.isGroupExi(group.getGroupName())) {
			...
			if(StringUtil.isBlank(tagName)){
				tagName=localtagService.getTagNamesByAnalysis(group.getGroupName()+","+group.getIntro());
				group.setTagNames(tagName);
			}
			
			//1.生成群号
			String groupNum=groupNumService.getGroupNum();
			//2.先创建openfire群组,以便于进行群组的聊天等
			group.setOpenFireGroup(groupService.creatOpenFireGroup(groupNum,group.getGroupName(),
			...
			if(group.getCategoryId()==null){
				group.setCategoryId(groupCateSupo);
			}
			if(group.getChildCategoryId()==null){
				group.setChildCategoryId(groupCateSec);
			}
			group=groupRepo.save(group);
			//将此群加入我的群组列表
			...
			groupService.creatGroupCreatDynamic(group.getId(), user.getId(), user.getNickName(), user.getLogoURL(), group.getCtime());
			try {
	...
	public XueWenGroup updateGroupPc(XueWenGroup group, User user)throws XueWenServiceException {
		...
		if (null == oldGroup) {
			...
			// 将修改的参数，复制到原群组对象
			if(group !=null){
				if(!oldGroup.getGroupName().equals(group.getGroupName()) || !oldGroup.getLogoUrl().equals(group.getLogoUrl())){
...
//						"isOpen","bgUrl"});
			...
			//判断群组的isopen
			String isOpen=group.getIsOpen();
			if(StringUtil.isBlank(isOpen)){
				oldGroup.setIsOpen("0");
			}else{
				oldGroup.setIsOpen(isOpen);
			}
//			if(isOpen==null || isOpen.equals("") || isOpen.equals("0")){
//				oldGroup.setIsOpen("0");
//			}else if(isOpen.equals("2")){
...
//	}
	 ...
	 * @throws
	 */
	public Map<String, Object> findByChildCategoryId(String childCategoryId,Pageable pageable) throws XueWenServiceException {
		Page<XueWenGroup> groups=null;
		if(StringUtil.isBlank(childCategoryId)){
			groups=groupRepo.findAll(pageable);
		}else {
			groups = groupRepo.findByChildCategoryId(childCategoryId,pageable);
		}
		List<JSONObject> jsonObjects=new ArrayList<JSONObject>();
		String[] includeKey = {"id","groupName","intro","logoUrl","memberCount","tagNames"};
		for (XueWenGroup xueWenGroup : groups) {
