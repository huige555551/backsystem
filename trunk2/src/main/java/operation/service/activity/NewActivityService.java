...
public class NewActivityService {
	 ...
	 */
	public NewActivity create(NewActivity newActivity, String image, User user,String groupId, 
			String groupName,String groupLogoUrl,String lat, String lng,String activityLat, String activityLng)throws XueWenServiceException {
		...
		// 规划地理位置信息
		if (null != activityLat && null != activityLng) {
			double[] position = new double[] { Double.parseDouble(activityLng),
					Double.parseDouble(activityLng) };
			newActivity.setActivityLng(Double.parseDouble(activityLng));
			newActivity.setActivityLat(Double.parseDouble(activityLat));
			newActivity.setActivityPosition(position);
		}
...
//    	}
	 ...
	 * @return
	 */
	public NewActivity update(String activityId, String group, String image,String lat, String lng, 
			User currUser,NewActivity activity,String activityLat,String activityLng) throws XueWenServiceException{
		NewActivity newActivity = newActivityRepo.findOne(activityId);
		if(newActivity == null){
			throw new XueWenServiceException(Config.STATUS_201, Config.MSG_NODATA_201,null);
		}
		...
		if(!StringUtil.isBlank(activity.getPrice())){
	 ...
	 * @throws XueWenServiceException
	 */
	public NewActivityUser signupFree(String activityId, User currUser,String groupId, String groupName, 
			String groupLogoUrl,String channel, String phoneNumber) throws XueWenServiceException{
		NewActivity activity = this.getOneById(activityId);
		return this.buyActivity(currUser.getId(), currUser.getNickName(), currUser.getLogoURL(), phoneNumber, groupId, groupName, groupLogoUrl, "", activity, activityId);
	 ...
	 * @throws XueWenServiceException
	 */
	public NewActivityUser buyActivity(String userId,String nickName,String logoUrl,String phoneNumber,
			String groupId,String groupName,String groupLogoUrl,String orderId,NewActivity activity,String activityId)throws XueWenServiceException{
		
		if(!this.isApply(userId, activityId)){
			NewActivityUser nau = new NewActivityUser();
			nau.setActivity(activity);
			nau.setActivityId(activityId);
			nau.setCtime(System.currentTimeMillis());
			nau.setGroupId(groupId);
			nau.setGroupLogoUrl(groupLogoUrl);
			nau.setGroupName(groupName);
			nau.setName(nickName);
			nau.setOrderId(orderId);
			nau.setPhone(phoneNumber);
			nau.setUserId(userId);
			nau.setUserLogoUrl(logoUrl);
			
			newActivityUserRepo.save(nau);
			newActivityTemplate.addBuyCount(activityId);
			
			try {
				ztiaoPushService.sendApplyActivity(userId, groupId, groupName, groupLogoUrl, activityId, activity.getName());
			} catch (Exception e) {
				l.error("================购买课程推送失败："+activityId);
			}
			return nau;
		}
		return null;
